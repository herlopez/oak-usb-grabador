...
# Máscara para blanco en HSV (ajusta si es necesario)
...
while True:
    if not paused or cv2.waitKey(1) & 0xFF == ord('n'):
        ...
        for c in contours:
            ...
            if area > MIN_AREA and rw > MIN_WIDTH and rh > MIN_HEIGHT:
        ...
        for obj in detectados:
            ...
            if not data['alertado'] and len(trayectoria) > FRAMES_QUIETOS:
                x_ini, y_ini = trayectoria[0]
                viene_de_fuera = (x_ini < 50 or y_ini < 50)

                ultimos = trayectoria[-FRAMES_QUIETOS:]
                velocidades = [np.linalg.norm(np.array(ultimos[i]) - np.array(ultimos[i-1])) for i in range(1, len(ultimos))]
                velocidad_prom = sum(velocidades) / len(velocidades)

                en_zona = zx <= centro[0] <= zx+zw and zy <= centro[1] <= zy+zh
                if viene_de_fuera and en_zona and velocidad_prom < VELOCIDAD_UMBRAL:
                    print(f"⚠️ Objeto ID {oid} COLOCADO en la zona de alerta")
                    cv2.putText(zona, "⚠️ ALERTA: OBJETO DETENIDO", (rx, ry+rh+25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2)
